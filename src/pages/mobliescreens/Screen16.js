    ...
    <div className="min-h-screen bg-black flex flex-col w-full max-w-md mx-auto px-4 pt-4">
      ...
      <div className="flex items-center mb-2">
        ...
        <div className="flex-1 flex flex-col items-center -ml-10">
          <span className="text-white font-semibold text-base leading-tight">Sell BTC</span>
          <span className="text-gray-300 text-xs font-semibold">COIN <span className="mx-1">|</span> Bitcoin</span>
        </div>
      ...
      <div className="flex flex-col items-center mt-10 mb-8">
        <span className="text-gray-300 text-sm mb-2">Balance: 0 BTC</span>
        <span className="text-white font-semibold text-4xl sm:text-5xl">{amount} BTC</span>
      </div>
      {/* Sell Button */}
      ...
      <div className="grid grid-cols-4 gap-5 max-w-xs mx-4 mb-2">
        {keypad.flat().map((key, i) => (
          <button
            key={i}
            className={`h-12 flex items-center justify-center text-white text-2xl font-bold rounded-full ${key === "ok" ? "bg-[#4f3cc9]" : key === "del" ? "" : key === "-" ? "" : ""}`}
